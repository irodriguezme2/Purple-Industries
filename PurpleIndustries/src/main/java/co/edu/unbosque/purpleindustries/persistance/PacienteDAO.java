...
public class PacienteDAO implements OperacionDAO<Paciente> {
	...
	public void cargarDesdeArchivo() {
		List<List<String>> datos = SheetsManager.leerDesdeSheets(TEXT_FILE_NAME, "A:R");
		...
		for (int i = 0; i < datos.size(); i++) {
			...
			if (fila.size() >= 11) {
				try {
					...
					int documento = Integer.parseInt(fila.get(4));
					String email = fila.get(5);
					double altura = Double.parseDouble(fila.get(6));
					double peso = Double.parseDouble(fila.get(7));
